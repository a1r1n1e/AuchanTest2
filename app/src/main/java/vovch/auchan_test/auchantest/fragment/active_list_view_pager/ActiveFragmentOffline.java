...
public class ActiveFragmentOffline extends Fragment {
    ...
    public void listsLayoutDrawer(SList list){
        ...
        Uri uri3 = Uri.parse("android.resource://vovch.auchan_test.auchantest/mipmap/resend_custom_white_green");
        ...
        Uri uri2 = Uri.parse("android.resource://vovch.auchan_test.auchantest/mipmap/redact_custom_white_green");
    ...
    private void onItemMarkButtonTouchedAction(ResultItemLayout layout){
        if(layout.getTrueItem() != null) {
            layout.setFocusable(false);
            layout.setClickable(false);
            ActiveListsActivity activity = (ActiveListsActivity) getActivity();
            if(activity != null) {
                activity.itemmark(layout.getTrueItem());
            }
        }
    }
    private void makeListogramLine(Item item, LinearLayout listogramLayout){
        ...
        ResultItemLayout resultItemLayout = (ResultItemLayout) LayoutInflater.from(addingFrameLayout.getContext()).inflate(R.layout.result_item_layout, addingFrameLayout, false);

        LinearLayout addingLayout = (LinearLayout) LayoutInflater.from(resultItemLayout.getContext()).inflate(R.layout.list_element_linear_layout, resultItemLayout, false);
        ...
        resultItemLayout.setTrueItem(item);
        resultItemLayout.setFocusable(true);
        resultItemLayout.setClickable(true);
        View.OnClickListener ItemMarkButtonListenner = new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                onItemMarkButtonTouchedAction((ResultItemLayout) v);
            }
        };
        if (item.getState()) {
            resultItemLayout.setOnClickListener(ItemMarkButtonListenner);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                resultItemLayout.setBackground(getActivity().getDrawable(R.drawable.no_corners_layout_color_1));
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                resultItemLayout.setBackground(getResources().getDrawable(R.drawable.no_corners_layout_color_1));
            } else {
                resultItemLayout.setBackgroundDrawable(getResources().getDrawable(R.drawable.no_corners_layout_color_1));
            }
        } else {
            resultItemLayout.setOnClickListener(ItemMarkButtonListenner);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                resultItemLayout.setBackground(getActivity().getDrawable(R.drawable.no_corners_layout_color_2));
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                resultItemLayout.setBackground(getResources().getDrawable(R.drawable.no_corners_layout_color_2));
            } else {
                resultItemLayout.setBackgroundDrawable(getResources().getDrawable(R.drawable.no_corners_layout_color_2));
            }
        }

        addingLayout.addView(itemName);
        item.setLayout(addingLayout);
        item.setVerticalLayout(resultItemLayout);
        //item.setButton(groupButton);

        resultItemLayout.addView(addingLayout);
        if(item.getImage() != null){
            ImageView imageView = (ImageView) LayoutInflater.from(resultItemLayout.getContext()).inflate(R.layout.item_image_view, resultItemLayout, false);
            imageView.setImageBitmap(item.getImage());
            resultItemLayout.addView(imageView);
        }

        addingFrameLayout.addView(resultItemLayout);
    ...
    public void fragmentShowItemmarkProcessing(Item item){
        LinearLayout layout = item.getVerticalLayout();
    ...
    public void fragmentShowSecondGood(Item result){
        ...
        if(itemMarkLayout != null && getActivity() != null) {
            ...
            ResultItemLayout itemMarkLayout = (ResultItemLayout) result.getVerticalLayout();
            if (itemMarkLayout != null) {
                itemMarkLayout.setFocusable(true);
                itemMarkLayout.setClickable(true);
                itemMarkLayout.setAlpha(1);
            }
            if(itemMarkLayout != null && getActivity() != null) {
                if (result.getState()) {
    ...
    public void fragmentShowSecondBad(Item result){
        ...
        ResultItemLayout layout = (ResultItemLayout) result.getVerticalLayout();
            if (layout != null) {
                layout.setAlpha(1);
